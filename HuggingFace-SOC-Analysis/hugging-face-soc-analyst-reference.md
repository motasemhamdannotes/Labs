# Hugging Face SOC Incident Analyst Reference

## Structured investigation writeup and operational cheat sheet

This reference converts the incident notes into an analyst-ready investigation narrative. The central lesson is simple: action volume does not equal impact. A defensible story emerges only after separating successful access from refused attempts and joining evidence across the agent trace, worker logs, malicious dataset configuration, C2 messages, captured environment data, and Kubernetes audit events.
